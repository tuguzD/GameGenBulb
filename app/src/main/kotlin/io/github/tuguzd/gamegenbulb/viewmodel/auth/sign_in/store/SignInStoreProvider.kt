...
import io.github.tuguzd.gamegenbulb.viewmodel.auth.sign_in.store.SignInStore.*
    ...
    private object ReducerImpl : Reducer<State, Message> {
        override fun State.reduce(msg: Message): State = when (msg) {
            is Message.NameChanged -> copy(
                name = msg.name, valid = msg.valid
            )
            is Message.PasswordChanged -> copy(
                password = msg.password, valid = msg.valid
            )
            is Message.PasswordVisibleChanged -> copy(
                passwordVisible = msg.passwordVisible
            )
